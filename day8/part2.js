...
directions = directions.split("").map(x=>x=="L"?0:1);
network = network.replace(/[()]/g,"");
network = network.split(/\n| = /g);
let navigate = {};
for (let nodeIndex = 0; nodeIndex < network.length; nodeIndex+=2) {
    navigate[network[nodeIndex]] = network[nodeIndex+1].split(", ")
}
// console.log(navigate)
let nodes = [];
let cycles = [];
for (const key of Object.keys(navigate)) {
...
for (let index = 0, nodelen = nodes.length; index < nodelen; index++) {
    let node = nodes[index];
    let i = 0;
    while(node[2] != "Z"){
        node = navigate[node][directions[i%directions.length]];
        i++;
    }
    cycles.push(i);
}
