const input = `Game 1: 7 blue, 5 red; 10 red, 7 blue; 5 blue, 4 green, 15 red; 4 green, 6 red, 7 blue; 5 green, 8 blue, 4 red; 5 red, 4 blue, 3 green
...
//2286 test set sum
//48, 12, 1560, 630, and 36 test set powers
...
for (let game of games) {
    ...
    data = data.splice(1);
    let sorted = {"red":[],"green":[],"blue":[]}
    for (let cubes of data){
        let cube = cubes.match(/\S+/g);
        sorted[cube[1]].push(cube[0]);        
    }
    let power = 1;
    Object.keys(sorted).forEach(color => {
        power *= Math.max(...sorted[color])
    });
    powerSum += power
}
