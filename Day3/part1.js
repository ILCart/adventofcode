...
for (let i = 0; i < rows.length; i++) {
    ...
    for (let symbol of symbols) {
        for (let direction of directions) {
            ...
            let originalIndex = (i + direction[1]) * (rows[i].length + 1) + (symbol.index + direction[0]);
...
partialPartNumberIndex = [...new Set(partialPartNumberIndex)];
let previousIndexed = [];
let sum = 0;
for (let digit of digits) {
    for (let partialPart of partialPartNumberIndex) {
        if (partialPart >= digit.index && partialPart <= digit.index + digit.length + 1 && !previousIndexed.includes(digit.index)) {
            previousIndexed.push(digit.index);
            sum += parseInt(digit[0],10);
            console.log(`Part Number found "${digit[0]}" @ ${digit.index} - ${digit.index + digit.length}`);
        }
    }
}
console.log(sum);