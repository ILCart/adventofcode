let input = `F.F77.F7F7FFJJ..77FF.--L777.-7777--F7F7FJFFF7FF|.F77FF|-7-|-F7-7-FF|FJ-FFJ7-F--F|-L--7L|-7FFFF---7-FJ.7J.F-7J.F|7-LF7L7|.F-L|7...LF--LL7F-77
...
function shoelace(set){
    let n = 0;
    let e = [...set.values()];
    let before = e[set.size-1].split(" ").map(x=>+x);
    let total = 0;
    while(set.size > 0){
        let xy = e[n];
        let [x,y] = xy.split(" ").map(x=>+x);
        set.delete(xy)
        total += (before[0]*y) - (x*before[1])
        before = [+x,+y]
        n++
    }
    // set.forEach(xy => {
...
function turn_to_cw(set){
    ...
    for (let i = 0; i < set_values.length; i+=2) {
        even.push(set_values[i]);
        if(set_values[i+1] != null) odd.push(set_values[i+1]);
    }
    
    return new Set(odd.reverse().concat(even))
}

let area = shoelace(turn_to_cw(visited));
